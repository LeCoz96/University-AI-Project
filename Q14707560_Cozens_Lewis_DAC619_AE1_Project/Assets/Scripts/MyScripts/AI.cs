...
{
    #region ProvidedAgentInformation
    // Gives access to important data about the AI agent (see above)
    public AgentData agentData;
    // Gives access to the agent senses
    public Sensing agentSenses;
    // gives access to the agents inventory
    public InventoryController agentInventory;
    // This is the script containing the AI agents actions
    // e.g. agentScript.MoveTo(enemy);
    public AgentActions agentActions;
    #endregion

    [Header("Behavior Tree Roots")]
    [SerializeField] private BTDefender _defenderScript;
    [SerializeField] private BTFlagRunner _flagRunnerScript;
    ...
    void Start()
    {
        ...
        _defenderScript = GetComponent<BTDefender>();
        _flagRunnerScript = GetComponent<BTFlagRunner>();

        PrimaryStateChoice();
    }

    private void PrimaryStateChoice() // The first member of each team is chosen to be a defender, the other flag runners
    {
        if (gameObject.tag == Tags.BlueTeam)
        {
            if (BlackBoard._numberOfBlueDefenders < BlackBoard._maxNumberOfDefenders)
            {
                ++BlackBoard._numberOfBlueDefenders;
                _defenderScript.enabled = true;
            }
            else
                _flagRunnerScript.enabled = true;
        }
        else if (gameObject.tag == Tags.RedTeam)
        {
            if (BlackBoard._numberOfRedDefenders < BlackBoard._maxNumberOfDefenders)
            {
                ++BlackBoard._numberOfRedDefenders;
                _defenderScript.enabled = true;
            }
            else
                _flagRunnerScript.enabled = true;
